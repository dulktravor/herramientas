...
## Bitácora: 2026-09-07 — Taller EPUB

- Inicio: leídas especificación completa, guía de despliegue y sistema de diseño. Repositorio confirmado, rama main y árbol limpio. Se implementará creación y edición local de EPUB con validación básica, navegación, extracción y límites explícitos. Se mantiene el despliegue GitHub → Cloudflare solicitado.

- Sincronización: primer intento impedido por permisos de .git; repetido con ejecución autorizada y completado, sin cambios remotos. Consultada especificación W3C EPUB 3.3 (https://www.w3.org/TR/epub-33/). Diseño: editor de capítulos y previsualización aislada, salida EPUB 3, rechazo explícito de DRM/maquetación fija, motores bajo demanda y compresión cancelable.
- Desarrollo: añadidos motor EPUB, Worker de ZIP con límites previos y durante inflación, editor, portada, recursos, índice y exportaciones; integrados directorio, portada, SEO y sitemap. Instalación inicial limitada por red, cancelada y repetida con autorización. Añadidos marked, DOMPurify y Playwright con archivo de bloqueo. Primera revisión de lint corrigió asociación de etiquetas, objetos URL locales y controles de rutas. Se están ejecutando pruebas funcionales antes de publicar.
- Pruebas iniciales: cuatro escenarios funcionales correctos (creación/relectura, sanitización/enlaces/portada, inválidos/limpieza y móvil/cancelación). Se sustituyó una imagen sintética dañada en la prueba y se tradujo el error de portada. Build correcto. La revisión tipada detectó promesas no marcadas en tres pruebas previas de fuentes; se añadieron void explícitos sin cambiar su lógica. Auditoría: solo aviso moderado preexistente en qs; las dependencias añadidas no presentan avisos. Inicio de servidor compilado restringido por permisos, repetido con autorización.
- Prueba ampliada: EPUB 2 importado, metadatos y recursos relativos conservados, referencias ausentes y fragmentos rotos detectados, rutas peligrosas bloqueadas y DRM rechazado. Detectado bloqueo de imágenes blob en iframe de origen aislado: sustituido por imágenes raster incrustadas localmente como data URL, con máximo de 10 MB en vista previa y cancelación de lectores. Cinco pruebas funcionales pasan en desarrollo; tres pruebas de fuentes también pasan. Se repite compilación y prueba final con instalación reproducible.
- Validación final: npm ci, lint, TypeScript, build y diff --check correctos. Las cinco pruebas EPUB pasan sobre la versión compilada: EPUB 2/3, creación Markdown/HTML, reordenación, exportación y reapertura, mimetype primero sin compresión, índice, TXT/ZIP, portada e imágenes relativas, sanitización, detección de enlaces y rutas inválidas, rechazo DRM, limpieza, cancelación y móvil/tema oscuro/teclado. No hubo solicitudes POST/PUT/PATCH durante el flujo de archivos. Remoto comprobado: main sin avances. Sin variables modificadas.
- Publicación iniciada: commit de producto 6211faea00c0f7c8d3aa72cfda072fd50e90f859 enviado a origin/main. Pendiente confirmar resultado de Cloudflare y ejecutar verificación pública. Capturas revisadas: escritorio claro, móvil oscuro y contenido de la vista previa visible; sin desbordamiento horizontal.

## Versión: 2026-09-07 — Taller EPUB local

- **Commit de producto:** `6211fae` (`feat: añadir Taller EPUB local con edición y validación`).
- **Tipo:** función, privacidad, seguridad, accesibilidad y SEO.
- **Cambios:** creación desde HTML/Markdown, apertura EPUB 2/3 adaptable, edición de título/autor/idioma/portada, capítulos editables y reordenables, recursos con rutas corregibles, índice EPUB 3 regenerado, detección de recursos ausentes y fragmentos rotos, validación básica antes de EPUB y extracción TXT/ZIP. Directorio, categoría Libros electrónicos, portada y sitemap integrados.
- **Variables modificadas:** ninguna.
- **Validaciones:** instalación reproducible npm ci, lint, TypeScript, build, diff --check, cinco pruebas EPUB sobre desarrollo, build y producción, y tres pruebas existentes de fuentes. Comprobados EPUB 2, creación/relectura EPUB 3, mimetype inicial sin compresión, navegación, metadatos, imágenes relativas y portada, extracción, sanitización, rutas inseguras, DRM, cancelación, limpieza, teclado, móvil y ambos temas. No se detectaron peticiones de envío en el flujo de archivos.
- **Despliegue:** automático GitHub → Cloudflare; Workers Builds y Cloudflare Pages completados con éxito para el commit de producto.
- **Resultado:** correcto; cinco pruebas pasan también en la versión pública.
- **URL verificada:** https://herramientas.enrique-lazaro-dulktravor.workers.dev/herramientas/epub.
